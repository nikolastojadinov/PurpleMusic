...
export const PlayerProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const savedQueue = Array.isArray(parsed.queue) ? parsed.queue.map(normalizeTrack).filter((t) => Boolean(t.youtubeVideoId)) : [];
      const savedIndex = clampIndex(parsed.index ?? 0, savedQueue);
      const active = savedQueue[savedIndex] || null;

      if (!active) return;

      setQueue(savedQueue);
      setCurrentIndex(savedIndex);
      setPlaybackContext((parsed.playbackContext as PlaybackContext) ?? "song");
      setYoutubePlaylistId(normalizeString(parsed.youtubePlaylistId) || null);
      ...
      resumeAtRef.current = Number(parsed.position ?? null);
...
export type { PlaybackContext, PlayRequest };
